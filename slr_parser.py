    ...
    def LR_parser(self, w):
        ...
        while True:
            ...
            if a not in self.parse_table[s]:
                results['action'].append(f'ERROR: unrecognized symbol {a} at position {pointer}')
                break

            elif not self.parse_table[s][a]:
                results['action'].append(f'ERROR: input cannot be parsed by given grammar at position {pointer}')
                break

            elif '/' in self.parse_table[s][a]:
                action = 'reduce' if self.parse_table[s][a].count('r') > 1 else 'shift'
                results['action'].append(f'ERROR: {action}-reduce conflict at state {s}, symbol {a} at position {pointer}')
                break
                ...
                pointer += 1
                a = buffer[pointer]
                parse_tree_stack.append(ParseNode(a))
                ...
                if body != ('^',):
                    ...
                    children = parse_tree_stack[-len(body):]
                    parse_tree_stack = parse_tree_stack[:-len(body)]
                else:
                    children = []

                node = ParseNode(head, children)
                parse_tree_stack.append(node)
        ...
        return results, parse_tree_stack[0] if parse_tree_stack else None
...
def print_parse_tree(node, indent=0):
    ...
    for child in node.children:
        result += print_parse_tree(child, indent + 1)
...
def main():
    ...
    parser.add_argument('tokens_file', type=argparse.FileType('r'), help='file containing tokens to be parsed')
    args = parser.parse_args()

    G = Grammar(args.grammar_file.read())
    slr_parser = SLRParser(G)
    slr_parser.print_info()
    results, parse_tree = slr_parser.LR_parser(args.tokens_file.read())
    slr_parser.print_LR_parser(results)

    if parse_tree:
        print("\nParse Tree:")
        tree_str = print_parse_tree(parse_tree)
        print(tree_str)
    else:
